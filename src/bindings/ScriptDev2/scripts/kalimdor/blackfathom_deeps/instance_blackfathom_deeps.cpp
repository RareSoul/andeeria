...
bool GOHello_go_shrine(Player* pPlayer, GameObject* _GO)
{
	...
	uint32 i = pInstance->GetData(DATA_COUNTER);
		i = i+1;
    pInstance->SetData(DATA_COUNTER,i);

	if(pInstance->GetData(DATA_COUNTER) == 4)
	{
		if(pInstance->GetData(TYPE_KELRIS) == DONE)
		pInstance->SetData(TYPE_SHRINE,DONE);
	}
    return false;
}
